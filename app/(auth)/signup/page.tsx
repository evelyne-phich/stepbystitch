...
import { Sparkles, Lock, Mail, User, AlertCircle, Loader2, Globe, CheckCircle2, ArrowLeft } from 'lucide-react';
...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    formData.set('languePreferee', selectedLang);
    ...
    <div className="min-h-screen flex flex-col justify-center py-12 sm:px-6 lg:px-8 bg-yarn-50">
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md px-4 sm:px-0">
        <div className="bg-white py-8 px-6 sm:px-10 rounded-3xl border border-yarn-200 shadow-lift">
          ...
          {successMessage && (
            <div className="mb-6 p-4 rounded-2xl bg-sage-50 border border-sage-200 text-sage-900 text-sm flex items-start gap-3">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label htmlFor="fullName" className="block text-xs font-semibold uppercase tracking-wider text-yarn-700 mb-1.5">
            ...
            <div>
              <label htmlFor="languePreferee" className="block text-xs font-semibold uppercase tracking-wider text-yarn-700 mb-1.5">
                {t.auth.preferredLangLabel}
              </label>
              <div className="relative rounded-2xl shadow-sm">
                <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-yarn-400">
                ...
                <select
                  ...
                  name="languePreferee"
                  defaultValue={locale}
                  className="block w-full pl-10 pr-4 py-3 rounded-2xl border border-yarn-300 text-yarn-900 focus:outline-none focus:ring-2 focus:ring-sage-500 focus:border-transparent text-sm bg-yarn-50/50"
                >
                  <option value="fr">{t.auth.langFr}</option>
                  <option value="en">{t.auth.langEn}</option>
                  <option value="es">{t.auth.langEs}</option>
                  <option value="de">{t.auth.langDe}</option>
                  <option value="it">{t.auth.langIt}</option>
                </select>
              </div>
              <p className="mt-1 text-[11px] text-yarn-500">
                {t.auth.preferredLangHint}
              </p>
            </div>
            ...
            <div className="pt-2">
              <button
                type="submit"
                disabled={loading}
                className="w-full flex items-center justify-center gap-2 py-3.5 px-4 rounded-2xl text-sm font-bold text-white bg-gradient-to-r from-sage-800 via-sage-700 to-sage-600 hover:from-sage-900 hover:to-sage-700 shadow-soft hover:shadow-lift transition-all disabled:opacity-50"
              >
                {loading ? (
                  <>
          ...
          <div className="mt-6 pt-6 border-t border-yarn-100 text-center">
            <p className="text-sm text-yarn-700">
              ...
              <Link href="/login" className="font-semibold text-yarn-900 hover:text-yarn-700 underline underline-offset-2">
                {t.auth.loginLink}
              </Link>
            </p>
          </div>
